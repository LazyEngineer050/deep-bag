...
// ── 5. fetch player news from ESPN injuries endpoint ──────────────────────
async function fetchNews() {
    log('Fetching player news...');
    const data = await get('site.api.espn.com', '/apis/site/v2/sports/basketball/nba/injuries');
    const newsMap = {};
    for (const team of data.injuries || []) {
        for (const item of team.injuries || []) {
            const name = item.athlete?.displayName;
            if (name && item.longComment) {
                newsMap[name] = item.longComment;
            }
        }
    }
    return newsMap;
...
`;
    ...
    // Write news.js
    const newsMap  = await fetchNews();
    const newsPath = path.join(__dirname, 'news.js');
    const newsJs   = `// Auto-generated by update-stats.js — ${timestamp}\n// Do not edit manually. Run: node update-stats.js\n\nwindow.PLAYER_NEWS = ${JSON.stringify(newsMap, null, 2)};\n`;
    fs.writeFileSync(newsPath, newsJs, 'utf8');
    console.log(`✓ Wrote news for ${Object.keys(newsMap).length} players to news.js`);
